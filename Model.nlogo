extensions [ gis ]
breed [ cohortes cohorte ]
breed [ ruminants ruminant ]

cohortes-own [
  Sv Ev Iv           ; compartiments EDO du vecteur
  effectif           ; Sv + Ev + Iv
  age-semaines
  gite-origine
  patch-gite         ; gite d'emergence (lecture du forçage)
  infectee?
]

ruminants-own [
  etat-h             ; "S" "E" "I" "R"
  jours-etat
  gestante?
]

globals [
  occupation-sol
  semaine annee saison
  intensite intensite-cs
  part-contre-saison
  ; --- EDO vecteurs ---
  rho-emergence mu-v nu-v q-vertical
  facteur-eau-mediocre duree-vie-moustique
  pop-moustiques
  ; --- couche hote ---
  nb-ruminants
  proba-piqure beta-hv
  nu-h gamma-h delta-h
  seuil-piqures-letal
  nouveaux-cas cumul-avortements cumul-morts
  avortements-semaine naissances-semaine
  morts-maladie-sem morts-ressources-sem morts-piqures-sem
]

patches-own [
  classe-id classe-base type-milieu qualite-eau
  contre-saison? canal? seuil est-grande-eau? coeur-eau?
]


to setup
  clear-all
  set occupation-sol gis:load-dataset "Delta_fleuve_senegal_100m_small.asc"
  gis:set-world-envelope gis:envelope-of occupation-sol
  gis:apply-raster occupation-sol classe-id

  ask patches [
    set classe-base ifelse-value ((classe-id >= 0) and (classe-id <= 8)) [classe-nom classe-id] ["hors-zone"]
    set type-milieu classe-base
    set qualite-eau ifelse-value (classe-base = "eau") ["bonne"] ["na"]
    set contre-saison? false
    set canal? false
    set seuil 0
  ]
  ask patches [ set est-grande-eau? false ]
  ask patches with [classe-base = "eau"] [
    if (count neighbors with [classe-base = "eau"]) > 4 [ set est-grande-eau? true ]
  ]
  ask patches [ set coeur-eau? false ]
  ask patches with [est-grande-eau?] [
    if all? patches in-radius 2 [classe-base = "eau"] [ set coeur-eau? true ]
  ]

  calculer-seuils-eaux
  set semaine 1  set annee 1  set part-contre-saison 0.15
  set intensite 0  set intensite-cs 0
  appliquer-etat-paysage
  colorer

  ; --- parametres EDO vecteurs ---
  set rho-emergence 15
  set mu-v 0.25
  set nu-v 1.5
  set q-vertical 0.005
  set facteur-eau-mediocre 3
  set duree-vie-moustique 9
  set pop-moustiques 0
  ask cohortes [ die ]

  ; --- parametres couche hote ---
  set nb-ruminants 150
  set proba-piqure 0.7
  set beta-hv 0.6
  set nu-h 1.0
  set gamma-h 0.4
  set delta-h 0.08
  set seuil-piqures-letal 400
  set nouveaux-cas 0
  set cumul-avortements 0
  set cumul-morts 0
  set avortements-semaine 0
  set naissances-semaine 0
  set morts-maladie-sem 0
  set morts-ressources-sem 0
  set morts-piqures-sem 0
  ask ruminants [ die ]
  creer-ruminants

  reset-ticks
end

to calculer-seuils-eaux
  ask patches with [classe-base = "eau" and not est-grande-eau?] [ set seuil -1 ]
  let couche patches with [classe-base = "eau" and not est-grande-eau?
                            and any? neighbors with [classe-base != "eau"]]
  let prof 1
  while [ any? couche ] [
    ask couche [ set seuil prof ]
    set couche patch-set [neighbors with [classe-base = "eau" and not est-grande-eau? and seuil = -1]] of couche
    set prof prof + 1
  ]
  let maxe max (list 1 (max [seuil] of patches with [classe-base = "eau" and not est-grande-eau?]))
  ask patches with [classe-base = "eau" and not est-grande-eau?] [ set seuil 1 - (seuil / maxe) ]
end


to go
  set semaine semaine + 1
  if semaine > 52 [ set semaine 1  set annee annee + 1 ]
  calculer-intensite
  if semaine = 45 [ lancer-contre-saison ]
  if semaine = 6  [ nettoyer-contre-saison ]
  appliquer-etat-paysage

  ; --- couche vectorielle (EDO) ---
  emergence-vecteurs
  integrer-edo-vecteurs
  deplacer-vecteurs
  mortalite-cohortes

  ; --- couche hote ---
  deplacer-ruminants
  amorcer-epidemie
  transmission-vecteur-hote
  evolution-ruminants
  renouvellement-cheptel
  colorer-vecteurs

  set pop-moustiques sum [effectif] of cohortes
  colorer
  tick
end

to calculer-intensite
  ifelse semaine <= 8 [
    set saison "saison seche"                 set intensite 0
  ] [ ifelse semaine <= 30 [
    set saison "pluies (montee)"              set intensite (semaine - 8) / 22
  ] [ ifelse semaine <= 44 [
    set saison "regression (fin des pluies)"  set intensite (44 - semaine) / 14
  ] [
    set saison "seche / contre-saison"        set intensite 0
  ] ] ]
  ifelse semaine >= 45 [ set intensite-cs min (list 1 ((semaine - 44) / 6)) ]
  [ set intensite-cs ifelse-value (semaine <= 6) [(6 - semaine) / 6] [0] ]
end


to appliquer-etat-paysage
  ask patches with [classe-base = "cultures"] [
    let niv ifelse-value contre-saison? [intensite-cs] [intensite]
    ifelse niv > 0.02 [ set type-milieu "cultures" ] [ set type-milieu "sol-nu" ]
  ]
  ask patches with [classe-base = "veg-inondee"] [
    set type-milieu ifelse-value (intensite > 0.3) ["veg-inondee"] ["prairie"]
  ]
  ask patches with [classe-base = "eau" and est-grande-eau?] [
    set type-milieu "eau"  set qualite-eau "bonne"
  ]
  repeat round ((1 - intensite) * 2) [
    ask patches with [type-milieu = "eau" and est-grande-eau? and not coeur-eau?] [
      if any? neighbors with [type-milieu != "eau"] [
        set type-milieu "prairie"  set qualite-eau "na"
      ]
    ]
  ]
  ask patches with [classe-base = "eau" and not est-grande-eau? and not canal? and not contre-saison?] [
    ifelse intensite >= seuil [ set type-milieu "eau"     set qualite-eau "bonne" ]
                              [ set type-milieu "prairie" set qualite-eau "na" ]
  ]
  ask patches with [canal? or (contre-saison? and classe-base = "eau")] [
    ifelse intensite-cs > 0.02 [ set type-milieu "eau"     set qualite-eau "mediocre" ]
                               [ set type-milieu "prairie" set qualite-eau "na" ]
  ]
end

to lancer-contre-saison
  let candidates patches with [classe-base = "cultures" and type-milieu = "sol-nu"]
  let n-foyers min (list (count candidates) round (part-contre-saison * 720))
  if n-foyers > 0 [
    ask n-of n-foyers candidates [ if not contre-saison? [ etendre-parcelle ] ]
  ]
  let foyers patches with [contre-saison?]
  if any? foyers [
    ask n-of (min (list 25 (count foyers))) foyers [
      let src min-one-of (patches with [type-milieu = "eau"]) [distance myself]
      if src != nobody [ tracer-canal src self ]
    ]
  ]
  ask patches with [classe-base = "eau" and not est-grande-eau?] [
    if any? patches in-radius 50 with [contre-saison? and classe-base = "cultures"] [
      set contre-saison? true  set type-milieu "eau"  set qualite-eau "mediocre"
    ]
  ]
end

to etendre-parcelle
  let reste (400 + random 600)
  let frontiere (patch-set self)
  while [ any? frontiere and reste > 0 ] [
    let p one-of frontiere
    ask p [ set contre-saison? true  set qualite-eau "mediocre" ]
    set reste reste - 1
    set frontiere (patch-set frontiere
      [neighbors4 with [classe-base = "cultures" and type-milieu = "sol-nu" and not contre-saison?]] of p)
      with [not contre-saison?]
    set frontiere frontiere with [self != p]
  ]
end

to tracer-canal [source cible]
  let courant source  let pas 0
  while [ ([distance cible] of courant) > 1 and pas < 200 ] [
    let suivant [min-one-of neighbors [distance cible]] of courant
    ask suivant [
      if type-milieu != "eau" and not est-grande-eau? [
        set canal? true  set type-milieu "eau"  set qualite-eau "mediocre"
      ]
    ]
    set courant suivant  set pas pas + 1
  ]
end

to nettoyer-contre-saison
  ask patches with [contre-saison? or canal?] [
    set contre-saison? false  set canal? false
    set type-milieu ifelse-value (classe-base = "eau") ["prairie"] ["sol-nu"]
    set qualite-eau "na"
  ]
end


to emergence-vecteurs
  let facteur-saison (0.3 + intensite)
  if facteur-saison < 0.35 [ stop ]

  let gites patches with [type-milieu = "eau"]
  if any? gites [
    let n-cohortes round (20 + 60 * intensite)
    ask n-of (min (list n-cohortes (count gites))) gites [
      let W surface-eau-locale
      let Lambda taux-recrutement W
      if Lambda > 1 [
        sprout-cohortes 1 [
          set patch-gite myself
          set Sv min (list 300 (Lambda * 4))
          set Ev 0
          set Iv 0
          set effectif Sv + Ev + Iv
          set age-semaines 0
          set gite-origine [type-gite] of myself
          set infectee? false
          set shape "bug"
          set size 4
        ]
      ]
    ]
  ]
end

to integrer-edo-vecteurs
  let dt 0.1
  repeat 10 [
    ask cohortes [
      let p ifelse-value (patch-gite != nobody and [type-milieu] of patch-gite = "eau")
                         [patch-gite] [patch-here]
      let W [surface-eau-locale] of p
      let Lambda taux-recrutement W

      let lambda-v 0

      let dSv ((1 - q-vertical) * Lambda) - (lambda-v * Sv) - (mu-v * Sv)
      let dEv (lambda-v * Sv) - ((nu-v + mu-v) * Ev)
      let dIv (q-vertical * Lambda) + (nu-v * Ev) - (mu-v * Iv)

      set Sv max (list 0 (Sv + dt * dSv))
      set Ev max (list 0 (Ev + dt * dEv))
      set Iv max (list 0 (Iv + dt * dIv))
      set effectif Sv + Ev + Iv
    ]
  ]
end

to-report taux-recrutement [W]
  let facteur-saison (0.3 + intensite)
  report rho-emergence * W * facteur-saison
end

to-report surface-eau-locale
  if type-milieu != "eau" [ report 0 ]
  let base 1
  if qualite-eau = "mediocre" [ set base base * facteur-eau-mediocre ]
  if est-grande-eau? [ set base base * 0.3 ]
  report base
end

to deplacer-vecteurs
  ask cohortes [
    let hote-proche min-one-of ruminants in-radius 20 [distance myself]
    ifelse hote-proche != nobody [
      face hote-proche
      fd 4 + random 8
    ] [
      rt random 360
      fd 8 + random 15
    ]
    set age-semaines age-semaines + 1
    set size 3 + (effectif / 150)
    if size > 6 [ set size 6 ]
  ]
end

to mortalite-cohortes
  ask cohortes [
    if effectif < 1 or age-semaines > (duree-vie-moustique + 5) [ die ]
  ]
end

to colorer-vecteurs
  ask cohortes [
    ; une cohorte devient infectee des qu'une part notable porte le virus,
    ; et le RESTE toute sa vie (les moustiques ne guerissent jamais)
    if effectif > 0 and (Iv / effectif) > 0.05 [ set infectee? true ]
    ifelse infectee? [
      set color red          ; nuee infectieuse (persistante)
    ] [
      set color cyan         ; nuee saine
    ]
  ]
end

to-report type-gite
  if qualite-eau = "mediocre" [ report "eau-mediocre" ]
  if est-grande-eau? [ report "grande-eau" ]
  report "mare-pluie"
end


to creer-ruminants
  let zones patches with [type-milieu = "prairie" or type-milieu = "arbustes"
                          or type-milieu = "sol-nu"]
  if not any? zones [ stop ]
  create-ruminants nb-ruminants [
    move-to one-of zones
    set etat-h "S"  set jours-etat 0
    set gestante? (random-float 1 < 0.4)
    set shape "cow"  set size 8  set color white
  ]
end

to deplacer-ruminants
  ask ruminants [
    let portee ifelse-value (intensite < 0.3) [12 + random 20] [4 + random 8]

    let destinations patches in-radius portee with [
      type-milieu = "prairie" or type-milieu = "arbustes"
      or (classe-base = "cultures" and type-milieu = "sol-nu")
    ]

    let bords destinations with [any? neighbors with [type-milieu = "eau"]]

    let cible nobody
    ifelse any? bords and random-float 1 < 0.4 [
      set cible one-of bords
    ] [
      if any? destinations [ set cible one-of destinations ]
    ]

    if cible != nobody [ move-to cible ]
  ]
end

to amorcer-epidemie
  if any? cohortes [
    if (count ruminants with [etat-h = "I" or etat-h = "E"]) = 0 [
      if any? ruminants with [etat-h = "S"] [
        ask n-of (min (list 5 (count ruminants with [etat-h = "S"]))) ruminants with [etat-h = "S"] [
          set etat-h "I"  set jours-etat 0
        ]
      ]
    ]
  ]
end

to transmission-vecteur-hote
  set nouveaux-cas 0

  ; ===== (A) HOTE INFECTIEUX -> VECTEURS : parcours par COHORTE =====
  ask cohortes [
    let n-inf count ruminants in-radius 8 with [etat-h = "I"]
    if n-inf > 0 [
      let proba-infection beta-hv * (1 - exp (0 - n-inf / 3))
      let nouveaux Sv * proba-infection * 0.35
      if nouveaux > Sv [ set nouveaux Sv ]
      set Sv max (list 0 (Sv - nouveaux))
      set Ev Ev + nouveaux                 ; -> exposé (incubation), pas Iv direct
      set effectif Sv + Ev + Iv
    ]
  ]

  ; ===== (B) VECTEURS INFECTIEUX -> HOTE : parcours par RUMINANT sain =====
  ask ruminants with [etat-h = "S"] [
    let coh cohortes in-radius 8 with [Iv > 0]
    if any? coh [
      let charge sum [Iv] of coh
      let lambda-h beta-vh * (1 - exp (0 - charge / 15))
      if random-float 1 < lambda-h [
        set etat-h "E"  set jours-etat 0
        set nouveaux-cas nouveaux-cas + 1
      ]
    ]
  ]
end

to evolution-ruminants
  set avortements-semaine 0
  set morts-maladie-sem 0
  set morts-ressources-sem 0
  set morts-piqures-sem 0
  ask ruminants [
    set jours-etat jours-etat + 1
    ; E -> I : au passage infectieux, TOUTE gestante avorte (signe FVR)
    if etat-h = "E" and jours-etat >= 1 [
      set etat-h "I"  set jours-etat 0
      if gestante? [
        set gestante? false
        set cumul-avortements cumul-avortements + 1
        set avortements-semaine avortements-semaine + 1
      ]
    ]
    ; I -> R / mort par maladie apres periode infectieuse
    if etat-h = "I" and jours-etat >= 5 [
      ifelse random-float 1 < delta-h [
        set cumul-morts cumul-morts + 1
        set morts-maladie-sem morts-maladie-sem + 1
        die
      ] [
        set etat-h "R"  set jours-etat 0
      ]
    ]
  ]

  ask ruminants with [etat-h = "I" or gestante?] [
    let charge-piqures sum [effectif] of cohortes in-radius 3
    if charge-piqures > seuil-piqures-letal [
      let p-mort 0.1 * (charge-piqures / seuil-piqures-letal - 1)
      if p-mort > 0.5 [ set p-mort 0.5 ]
      if random-float 1 < p-mort [
        set cumul-morts cumul-morts + 1
        set morts-piqures-sem morts-piqures-sem + 1
        die
      ]
    ]
  ]
  ; couleurs SEIR
  ask ruminants [
    if etat-h = "S" [ set color white ]
    if etat-h = "E" [ set color yellow ]
    if etat-h = "I" [ set color red ]
    if etat-h = "R" [ set color lime ]
  ]
end


to renouvellement-cheptel
  set naissances-semaine 0

  let facteur-saison ifelse-value (semaine >= 35 and semaine <= 46) [6] [1]
  let taux-naissance (taux-renouvellement-base * facteur-saison)
  let meres ruminants with [etat-h != "I" and etat-h != "E"]
  ask meres [
    if random-float 1 < taux-naissance [
      hatch-ruminants 1 [
        set etat-h "S"  set jours-etat 0
        set gestante? false
        set shape "cow"  set size 8  set color white
        rt random 360  fd 1
      ]
      set naissances-semaine naissances-semaine + 1
    ]
  ]

  if semaine >= 30 and semaine <= 44 [
    ask ruminants with [not gestante? and (etat-h = "S" or etat-h = "R")] [
      if random-float 1 < 0.15 [ set gestante? true ]
    ]
  ]

  ; mortalite naturelle saisonniere (rarete des ressources en saison seche)
  let mortalite-ressources (0.005 + 0.06 * (1 - intensite))
  ask ruminants [
    if random-float 1 < mortalite-ressources [
      set cumul-morts cumul-morts + 1
      set morts-ressources-sem morts-ressources-sem + 1
      die
    ]
  ]
end


to-report classe-nom [code]
  report item code ["eau" "arbres" "prairie" "veg-inondee" "cultures"
                    "arbustes" "bati" "sol-nu"]
end

to colorer
  ask patches [
    if type-milieu = "eau"         [ set pcolor ifelse-value (qualite-eau = "mediocre") [88] [104] ]
    if type-milieu = "cultures"    [ set pcolor couleur-culture ifelse-value contre-saison? [intensite-cs] [intensite] ]
    if type-milieu = "arbres"      [ set pcolor 53  ]
    if type-milieu = "prairie"     [ set pcolor 56  ]
    if type-milieu = "veg-inondee" [ set pcolor 103 ]
    if type-milieu = "arbustes"    [ set pcolor 46  ]
    if type-milieu = "bati"        [ set pcolor 1   ]
    if type-milieu = "sol-nu"      [ set pcolor 38  ]
    if type-milieu = "hors-zone"   [ set pcolor black ]
  ]
end

to-report couleur-culture [niveau]
  report scale-color green niveau 1.4 -0.2
end

to-report pct [t]
  report precision (100 * count patches with [type-milieu = t] / count patches) 1
end

to-report total-moustiques report pop-moustiques end
to-report nb-gites-eau report count patches with [type-milieu = "eau"] end
to-report moustiques-eau-mediocre report sum [effectif] of cohortes with [gite-origine = "eau-mediocre"] end
to-report moustiques-mare-pluie   report sum [effectif] of cohortes with [gite-origine = "mare-pluie"] end
to-report moustiques-grande-eau   report sum [effectif] of cohortes with [gite-origine = "grande-eau"] end
to-report moustiques-infectieux   report precision (sum [Iv] of cohortes) 0 end
to-report taux-infection-vecteurs
  let tot sum [effectif] of cohortes
  if tot = 0 [ report 0 ]
  report precision (100 * (sum [Iv] of cohortes) / tot) 1
end
to-report nb-S report count ruminants with [etat-h = "S"] end
to-report nb-E report count ruminants with [etat-h = "E"] end
to-report nb-I report count ruminants with [etat-h = "I"] end
to-report nb-R report count ruminants with [etat-h = "R"] end
to-report cas-hebdo report nouveaux-cas end
to-report avortements-hebdo report avortements-semaine end
to-report naissances-hebdo report naissances-semaine end
to-report morts-maladie-hebdo report morts-maladie-sem end
to-report morts-ressources-hebdo report morts-ressources-sem end
to-report morts-piqures-hebdo report morts-piqures-sem end
to-report morts-hebdo report morts-maladie-sem + morts-ressources-sem + morts-piqures-sem end
to-report population-cheptel report count ruminants end
@#$#@#$#@
GRAPHICS-WINDOW
91
10
1019
572
-1
-1
1.0
1
10
1
1
1
0
0
0
1
0
919
0
552
0
0
1
ticks
30.0

BUTTON
9
130
72
163
setup
setup
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

BUTTON
13
59
76
92
go
go
T
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

PLOT
1030
10
1361
130
Moustiques par gîte
effectif
gite-origine
0.0
10.0
0.0
10.0
true
true
"" ""
PENS
"moustiques-eau-mediocre" 1.0 0 -6459832 true "" "plot moustiques-eau-mediocre"
"moustiques-mare-pluie" 1.0 0 -13403783 true "" "plot moustiques-mare-pluie"
"moustiques-grande-eau" 1.0 0 -14730904 true "" "plot moustiques-grande-eau"

PLOT
1029
329
1427
554
Etats Ruminant
semaines
nombre de ruminants
0.0
10.0
0.0
10.0
true
true
"" ""
PENS
"Sains" 1.0 0 -13791810 true "" "plot nb-S"
"Infectés" 1.0 0 -2674135 true "" "plot nb-I"
"Rétablie" 1.0 0 -14439633 true "" "plot nb-R"

SLIDER
581
615
886
648
taux-renouvellement-base
taux-renouvellement-base
0
0.1
0.05
0.01
1
NIL
HORIZONTAL

PLOT
1026
555
1438
806
plot 1
semaines
nombre de morts
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -5298144 true "" "plot morts-maladie-hebdo"
" Mort Ressource" 1.0 0 -7171555 true "" "plot morts-ressources-hebdo"
"Mort piqure" 1.0 0 -6917194 true "" "plot morts-piqures-hebdo"

SLIDER
581
663
887
696
beta-vh
beta-vh
0
100
26.0
1
1
NIL
HORIZONTAL

PLOT
1028
144
1421
326
Naissance et mort 
semaines
nombre d'individus
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -2674135 true "" "plot avortements-hebdo"
"pen-1" 1.0 0 -13840069 true "" "plot naissances-hebdo"

@#$#@#$#@
## WHAT IS IT?

(a general understanding of what the model is trying to show or explain)

## HOW IT WORKS

(what rules the agents use to create the overall behavior of the model)

## HOW TO USE IT

(how to use the model, including a description of each of the items in the Interface tab)

## THINGS TO NOTICE

(suggested things for the user to notice while running the model)

## THINGS TO TRY

(suggested things for the user to try to do (move sliders, switches, etc.) with the model)

## EXTENDING THE MODEL

(suggested things to add or change in the Code tab to make the model more complicated, detailed, accurate, etc.)

## NETLOGO FEATURES

(interesting or unusual features of NetLogo that the model uses, particularly in the Code tab; or where workarounds were needed for missing features)

## RELATED MODELS

(models in the NetLogo Models Library and elsewhere which are of related interest)

## CREDITS AND REFERENCES

(a reference to the model's URL on the web if it has one, as well as any other necessary credits, citations, and links)
@#$#@#$#@
default
true
0
Polygon -7500403 true true 150 5 40 250 150 205 260 250

airplane
true
0
Polygon -7500403 true true 150 0 135 15 120 60 120 105 15 165 15 195 120 180 135 240 105 270 120 285 150 270 180 285 210 270 165 240 180 180 285 195 285 165 180 105 180 60 165 15

arrow
true
0
Polygon -7500403 true true 150 0 0 150 105 150 105 293 195 293 195 150 300 150

box
false
0
Polygon -7500403 true true 150 285 285 225 285 75 150 135
Polygon -7500403 true true 150 135 15 75 150 15 285 75
Polygon -7500403 true true 15 75 15 225 150 285 150 135
Line -16777216 false 150 285 150 135
Line -16777216 false 150 135 15 75
Line -16777216 false 150 135 285 75

bug
true
0
Circle -7500403 true true 96 182 108
Circle -7500403 true true 110 127 80
Circle -7500403 true true 110 75 80
Line -7500403 true 150 100 80 30
Line -7500403 true 150 100 220 30

butterfly
true
0
Polygon -7500403 true true 150 165 209 199 225 225 225 255 195 270 165 255 150 240
Polygon -7500403 true true 150 165 89 198 75 225 75 255 105 270 135 255 150 240
Polygon -7500403 true true 139 148 100 105 55 90 25 90 10 105 10 135 25 180 40 195 85 194 139 163
Polygon -7500403 true true 162 150 200 105 245 90 275 90 290 105 290 135 275 180 260 195 215 195 162 165
Polygon -16777216 true false 150 255 135 225 120 150 135 120 150 105 165 120 180 150 165 225
Circle -16777216 true false 135 90 30
Line -16777216 false 150 105 195 60
Line -16777216 false 150 105 105 60

car
false
0
Polygon -7500403 true true 300 180 279 164 261 144 240 135 226 132 213 106 203 84 185 63 159 50 135 50 75 60 0 150 0 165 0 225 300 225 300 180
Circle -16777216 true false 180 180 90
Circle -16777216 true false 30 180 90
Polygon -16777216 true false 162 80 132 78 134 135 209 135 194 105 189 96 180 89
Circle -7500403 true true 47 195 58
Circle -7500403 true true 195 195 58

circle
false
0
Circle -7500403 true true 0 0 300

circle 2
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240

cow
false
0
Polygon -7500403 true true 200 193 197 249 179 249 177 196 166 187 140 189 93 191 78 179 72 211 49 209 48 181 37 149 25 120 25 89 45 72 103 84 179 75 198 76 252 64 272 81 293 103 285 121 255 121 242 118 224 167
Polygon -7500403 true true 73 210 86 251 62 249 48 208
Polygon -7500403 true true 25 114 16 195 9 204 23 213 25 200 39 123

cylinder
false
0
Circle -7500403 true true 0 0 300

dot
false
0
Circle -7500403 true true 90 90 120

face happy
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 255 90 239 62 213 47 191 67 179 90 203 109 218 150 225 192 218 210 203 227 181 251 194 236 217 212 240

face neutral
false
0
Circle -7500403 true true 8 7 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Rectangle -16777216 true false 60 195 240 225

face sad
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 168 90 184 62 210 47 232 67 244 90 220 109 205 150 198 192 205 210 220 227 242 251 229 236 206 212 183

fish
false
0
Polygon -1 true false 44 131 21 87 15 86 0 120 15 150 0 180 13 214 20 212 45 166
Polygon -1 true false 135 195 119 235 95 218 76 210 46 204 60 165
Polygon -1 true false 75 45 83 77 71 103 86 114 166 78 135 60
Polygon -7500403 true true 30 136 151 77 226 81 280 119 292 146 292 160 287 170 270 195 195 210 151 212 30 166
Circle -16777216 true false 215 106 30

flag
false
0
Rectangle -7500403 true true 60 15 75 300
Polygon -7500403 true true 90 150 270 90 90 30
Line -7500403 true 75 135 90 135
Line -7500403 true 75 45 90 45

flower
false
0
Polygon -10899396 true false 135 120 165 165 180 210 180 240 150 300 165 300 195 240 195 195 165 135
Circle -7500403 true true 85 132 38
Circle -7500403 true true 130 147 38
Circle -7500403 true true 192 85 38
Circle -7500403 true true 85 40 38
Circle -7500403 true true 177 40 38
Circle -7500403 true true 177 132 38
Circle -7500403 true true 70 85 38
Circle -7500403 true true 130 25 38
Circle -7500403 true true 96 51 108
Circle -16777216 true false 113 68 74
Polygon -10899396 true false 189 233 219 188 249 173 279 188 234 218
Polygon -10899396 true false 180 255 150 210 105 210 75 240 135 240

house
false
0
Rectangle -7500403 true true 45 120 255 285
Rectangle -16777216 true false 120 210 180 285
Polygon -7500403 true true 15 120 150 15 285 120
Line -16777216 false 30 120 270 120

leaf
false
0
Polygon -7500403 true true 150 210 135 195 120 210 60 210 30 195 60 180 60 165 15 135 30 120 15 105 40 104 45 90 60 90 90 105 105 120 120 120 105 60 120 60 135 30 150 15 165 30 180 60 195 60 180 120 195 120 210 105 240 90 255 90 263 104 285 105 270 120 285 135 240 165 240 180 270 195 240 210 180 210 165 195
Polygon -7500403 true true 135 195 135 240 120 255 105 255 105 285 135 285 165 240 165 195

line
true
0
Line -7500403 true 150 0 150 300

line half
true
0
Line -7500403 true 150 0 150 150

pentagon
false
0
Polygon -7500403 true true 150 15 15 120 60 285 240 285 285 120

person
false
0
Circle -7500403 true true 110 5 80
Polygon -7500403 true true 105 90 120 195 90 285 105 300 135 300 150 225 165 300 195 300 210 285 180 195 195 90
Rectangle -7500403 true true 127 79 172 94
Polygon -7500403 true true 195 90 240 150 225 180 165 105
Polygon -7500403 true true 105 90 60 150 75 180 135 105

plant
false
0
Rectangle -7500403 true true 135 90 165 300
Polygon -7500403 true true 135 255 90 210 45 195 75 255 135 285
Polygon -7500403 true true 165 255 210 210 255 195 225 255 165 285
Polygon -7500403 true true 135 180 90 135 45 120 75 180 135 210
Polygon -7500403 true true 165 180 165 210 225 180 255 120 210 135
Polygon -7500403 true true 135 105 90 60 45 45 75 105 135 135
Polygon -7500403 true true 165 105 165 135 225 105 255 45 210 60
Polygon -7500403 true true 135 90 120 45 150 15 180 45 165 90

sheep
false
15
Circle -1 true true 203 65 88
Circle -1 true true 70 65 162
Circle -1 true true 150 105 120
Polygon -7500403 true false 218 120 240 165 255 165 278 120
Circle -7500403 true false 214 72 67
Rectangle -1 true true 164 223 179 298
Polygon -1 true true 45 285 30 285 30 240 15 195 45 210
Circle -1 true true 3 83 150
Rectangle -1 true true 65 221 80 296
Polygon -1 true true 195 285 210 285 210 240 240 210 195 210
Polygon -7500403 true false 276 85 285 105 302 99 294 83
Polygon -7500403 true false 219 85 210 105 193 99 201 83

square
false
0
Rectangle -7500403 true true 30 30 270 270

square 2
false
0
Rectangle -7500403 true true 30 30 270 270
Rectangle -16777216 true false 60 60 240 240

star
false
0
Polygon -7500403 true true 151 1 185 108 298 108 207 175 242 282 151 216 59 282 94 175 3 108 116 108

target
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240
Circle -7500403 true true 60 60 180
Circle -16777216 true false 90 90 120
Circle -7500403 true true 120 120 60

tree
false
0
Circle -7500403 true true 118 3 94
Rectangle -6459832 true false 120 195 180 300
Circle -7500403 true true 65 21 108
Circle -7500403 true true 116 41 127
Circle -7500403 true true 45 90 120
Circle -7500403 true true 104 74 152

triangle
false
0
Polygon -7500403 true true 150 30 15 255 285 255

triangle 2
false
0
Polygon -7500403 true true 150 30 15 255 285 255
Polygon -16777216 true false 151 99 225 223 75 224

truck
false
0
Rectangle -7500403 true true 4 45 195 187
Polygon -7500403 true true 296 193 296 150 259 134 244 104 208 104 207 194
Rectangle -1 true false 195 60 195 105
Polygon -16777216 true false 238 112 252 141 219 141 218 112
Circle -16777216 true false 234 174 42
Rectangle -7500403 true true 181 185 214 194
Circle -16777216 true false 144 174 42
Circle -16777216 true false 24 174 42
Circle -7500403 false true 24 174 42
Circle -7500403 false true 144 174 42
Circle -7500403 false true 234 174 42

turtle
true
0
Polygon -10899396 true false 215 204 240 233 246 254 228 266 215 252 193 210
Polygon -10899396 true false 195 90 225 75 245 75 260 89 269 108 261 124 240 105 225 105 210 105
Polygon -10899396 true false 105 90 75 75 55 75 40 89 31 108 39 124 60 105 75 105 90 105
Polygon -10899396 true false 132 85 134 64 107 51 108 17 150 2 192 18 192 52 169 65 172 87
Polygon -10899396 true false 85 204 60 233 54 254 72 266 85 252 107 210
Polygon -7500403 true true 119 75 179 75 209 101 224 135 220 225 175 261 128 261 81 224 74 135 88 99

wheel
false
0
Circle -7500403 true true 3 3 294
Circle -16777216 true false 30 30 240
Line -7500403 true 150 285 150 15
Line -7500403 true 15 150 285 150
Circle -7500403 true true 120 120 60
Line -7500403 true 216 40 79 269
Line -7500403 true 40 84 269 221
Line -7500403 true 40 216 269 79
Line -7500403 true 84 40 221 269

wolf
false
0
Polygon -16777216 true false 253 133 245 131 245 133
Polygon -7500403 true true 2 194 13 197 30 191 38 193 38 205 20 226 20 257 27 265 38 266 40 260 31 253 31 230 60 206 68 198 75 209 66 228 65 243 82 261 84 268 100 267 103 261 77 239 79 231 100 207 98 196 119 201 143 202 160 195 166 210 172 213 173 238 167 251 160 248 154 265 169 264 178 247 186 240 198 260 200 271 217 271 219 262 207 258 195 230 192 198 210 184 227 164 242 144 259 145 284 151 277 141 293 140 299 134 297 127 273 119 270 105
Polygon -7500403 true true -1 195 14 180 36 166 40 153 53 140 82 131 134 133 159 126 188 115 227 108 236 102 238 98 268 86 269 92 281 87 269 103 269 113

x
false
0
Polygon -7500403 true true 270 75 225 30 30 225 75 270
Polygon -7500403 true true 30 75 75 30 270 225 225 270
@#$#@#$#@
NetLogo 6.4.0
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
default
0.0
-0.2 0 0.0 1.0
0.0 1 1.0 0.0
0.2 0 0.0 1.0
link direction
true
0
Line -7500403 true 150 150 90 180
Line -7500403 true 150 150 210 180
@#$#@#$#@
0
@#$#@#$#@
